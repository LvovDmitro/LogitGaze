...
def get_args_parser_test():
    ...
    parser.add_argument('--use_logit_lens', default=False, action='store_true', help="Use logit lens semantic vectors")
    parser.add_argument(
        '--logit_lens_dir',
        default='./data/logit_lens_TP',
        type=str,
        help="Root directory with logit lens vectors (must contain 'logits/' and 'semantics/' subfolders)",
    )
    parser.add_argument('--logit_lens_top_k', default=5, type=int, help="Number of top tokens to use from logit lens (1-5)")
    parser.add_argument('--logit_lens_dim', default=4096, type=int, help="Embedding dimension of logit lens vectors")
...
def get_args_parser_train():
    ...
    parser.add_argument(
        '--logit_lens_dir',
        default='./reorganized/data/logit_lens_TP',
        type=str,
        help="Root directory with logit lens vectors (must contain 'logits/' and 'semantics/' subfolders)",
    )
    parser.add_argument('--logit_lens_top_k', default=5, type=int, help="Number of top tokens to use from logit lens (1-5)")
    parser.add_argument('--logit_lens_dim', default=4096, type=int, help="Embedding dimension of logit lens vectors")
    parser.add_argument('--logit_lens_layer', default='last', type=str, help="Which layer to use: 'last', 'middle', or layer index")
    parser.add_argument('--logit_lens_aggregation', default='mean', type=str, choices=['mean', 'weighted', 'top1'], help="How to aggregate top_k vectors: mean, weighted by probs, or top1 only")
