...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, img_hidden_dim = 2048, lm_dmodel = 768, nhead=8, num_encoder_layers=6,
                 ...
                 use_logit_lens=False, logit_lens_dim=4096, logit_lens_top_k=5):
        super().__init__()
        self.device = device
        ...
        if use_logit_lens:
            logit_lens_proj = nn.Linear(logit_lens_dim, d_model).to(device)
            logit_lens_attention = nn.MultiheadAttention(d_model, nhead, dropout=encoder_dropout).to(device)
        else:
            logit_lens_proj = None
            logit_lens_attention = None
        
        self.encoder =  TransformerEncoderWrapper(
            encoder, input_proj, img_transform, text_transform, encoder_dropout, device,
            use_logit_lens=use_logit_lens, logit_lens_proj=logit_lens_proj, 
            logit_lens_attention=logit_lens_attention, logit_lens_top_k=logit_lens_top_k
        ).to(device)
    ...
    def forward(self, src: Tensor, tgt: Tensor, task:Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
        ...
        memory, task_emb = self.encoder(
            src, mask=src_mask, task = task, src_key_padding_mask=src_key_padding_mask, 
            patchpos_embed=patchpos_embed, logit_lens_vectors=logit_lens_vectors
        )
...
class TransformerEncoderWrapper(nn.Module):
    ...
    def __init__(self, encoder, input_proj, img_transform,  text_transform, dropout, device,
                 use_logit_lens=False, logit_lens_proj=None, logit_lens_attention=None, logit_lens_top_k=5):
        super().__init__()
        self.device = device
        ...
        self.use_logit_lens = use_logit_lens
        self.logit_lens_proj = logit_lens_proj
        self.logit_lens_attention = logit_lens_attention
        ...
        if use_logit_lens and logit_lens_proj is not None:
            self._reset_parameters(self.logit_lens_proj)
    ...
    def forward(self, src, task,
                ...
                logit_lens_vectors: Optional[Tensor] = None):
        # project image features from img_hidden_dim -> d_model
        src_proj = self.input_proj(src).permute(1, 0, 2)

        if self.use_logit_lens and logit_lens_vectors is not None:
            batch_size = src_proj.size(1)
            seq_len = src_proj.size(0)

            # support both (num_patches, top_k, dim) and (B, num_patches, top_k, dim)
            if len(logit_lens_vectors.shape) == 3:
                num_patches = logit_lens_vectors.size(0)
                logit_lens_vectors = logit_lens_vectors.unsqueeze(0)
            else:
                num_patches = logit_lens_vectors.size(1)

            # aggregate over top_k tokens (mean over token dimension)
            logit_lens_mean = logit_lens_vectors.mean(dim=2)  # (B, num_patches, embedding_dim)

            # project to d_model
            logit_lens_proj = self.logit_lens_proj(logit_lens_mean)  # (B, num_patches, d_model)

            # (num_patches, B, d_model) to match src_proj
            logit_lens_proj = logit_lens_proj.permute(1, 0, 2)

            # add semantic features to visual features
            min_len = min(seq_len, logit_lens_proj.size(0))
            src_proj[:min_len] = src_proj[:min_len] + logit_lens_proj[:min_len]
        ...
        # project encoder output and task embedding to a common multimodal space
        memory = self.img_transform(output)
        task_emb = self.text_transform(task)
