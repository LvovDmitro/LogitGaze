...
class gazeformer(nn.Module):
    ...
    def forward(self, src: Tensor, tgt: Tensor, task: Tensor, logit_lens_vectors: Optional[Tensor] = None):
        ...
        outs = self.transformer(
            src=src, tgt=tgt_input, tgt_mask= None, tgt_key_padding_mask = None, 
            task = task.to(self.device), querypos_embed = self.querypos_embed.weight.unsqueeze(1), 
            patchpos_embed = self.patchpos_embed, logit_lens_vectors=logit_lens_vectors
        )
