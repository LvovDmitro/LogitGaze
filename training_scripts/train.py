...
# Ensure project root is on sys.path so that `models` and `utils` can be imported
import sys
...
def train(epoch, args, model, SlowOpt, MidOpt, FastOpt, loss_fn_token, loss_fn_y, loss_fn_x, loss_fn_t, train_dataloader, model_dir, model_name, device = 'cuda:0', im_h=20, im_w=32, patch_size=16):
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        ...
        for batch_data in tepoch:
            if len(batch_data) == 6:
                batch_imgs, batch_tgt, batch_tgt_padding_mask, batch_tasks, batch_firstfix, batch_logit_lens = batch_data
            else:
                batch_imgs, batch_tgt, batch_tgt_padding_mask, batch_tasks, batch_firstfix = batch_data
                batch_logit_lens = None
            
            out_token, out_y, out_x, out_t = model(
                src = batch_imgs, 
                tgt = batch_firstfix, 
                task = batch_tasks,
                logit_lens_vectors = batch_logit_lens
            )
...
def main(args):
    ...
    train_dataset = fixation_dataset(
        seq_train, 
        img_ftrs_dir = args.img_ftrs_dir,
        logit_lens_dir = args.logit_lens_dir if args.use_logit_lens else None,
        logit_lens_top_k = args.logit_lens_top_k
    )
    valid_dataset = fixation_dataset(
        seq_valid, 
        img_ftrs_dir = args.img_ftrs_dir,
        logit_lens_dir = args.logit_lens_dir if args.use_logit_lens else None,
        logit_lens_top_k = args.logit_lens_top_k
    ...
    collate_fn = COCOSearch18Collator(
        embedding_dict, args.max_len, args.im_h, args.im_w, args.patch_size,
    ...
    valid_dataloader = DataLoader(valid_dataset, batch_size = args.batch_size, shuffle=False, num_workers=0, collate_fn = collate_fn)
    ...
    transformer = Transformer(
        ...
        device = device,
        use_logit_lens=args.use_logit_lens,
        logit_lens_dim=args.logit_lens_dim,
        logit_lens_top_k=args.logit_lens_top_k
    ).to(device)
