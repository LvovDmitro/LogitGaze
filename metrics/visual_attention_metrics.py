...
This measures how well the saliencyMap of an image predicts the ground truth expert 
...
def AUC_Judd(saliencyMap, fixationMap, jitter=True, toPlot=False, msg=False):
    ...
    # fixationMap is the expert fixation map (binary matrix)
    ...
    for i in range(Nfixations):
        ...
        tp[i + 1] = (
            float(i + 1) / Nfixations
        )  # ratio sal map values at fixation locations
        # above threshold
        fp[i + 1] = float(aboveth - i) / (
            Npixels - Nfixations
    ...
    if toPlot:
        ...
        plt.axis((-0.5, s[1] - 0.5, s[0] - 0.5, -0.5))
        plt.plot(x, y, "ro")
...
def KLdiv(saliencyMap, fixationMap):
    # saliencyMap is the saliency map
    # fixationMap is the expert fixation map
...
NSS is the average of the response values at expert eye positions in a model saliency 
...
def _scanpath_to_string(scanpath, height, width, n):
    ...
    for i in range(np.shape(scanpath)[0]):
        ...
        correspondent_square = (fixation[0] // width_step) + (
            fixation[1] // height_step
        ) * n
        string += chr(97 + correspondent_square)

    return string

def calculate_scanmatch_score(human_scanpaths, predictions, with_duration=False):
    """
    Compute ScanMatch score between human and predicted scanpaths.
    Each entry in predictions should be a tuple of length 7:
    (X, Y, T, task_name, img_name, condition, subject)
    """

    # Create ScanMatch object
    scanmatch = ScanMatch(
        Xres=576, Yres=576, Xbin=8, Ybin=6,
        Offset=(0, 0),
        TempBin=100 if with_duration else 0,
        Threshold=3.5
    )
    ...
    for pred in predictions:
        # Unpack the prediction tuple in the correct order
        fixs_x = pred['X']
        fixs_y = pred['Y']
        fixs_t = pred['T']
        task_name = pred['task']
        ...
        # Pack the fixation data into a (T, 3) array
        fixs = np.stack([fixs_y, fixs_x, fixs_t], axis=1)
        
        # Find the human scanpaths that match the current prediction
        human_matches = [
            h for h in human_scanpaths
            if h['name'] == img_name and h['task'] == task_name and h['condition'] == condition
        ]

        if not human_matches:
            print("skipping no matching human scanpath")
            continue  # Skip if no matching human scanpath

        if fixs.shape[0] == 0:
            print("skipping no matching predictions")
            continue  # Skip empty predictions

        # Handle duration if required
        pred_data = fixs[:, [1, 0, 2]] if with_duration else fixs[:, [1, 0]]
        pred_seq = scanmatch.fixationToSequence(pred_data).astype(int)

        # Compare against each matching human scanpath
        for human in human_matches:
            human_fixs = np.stack([human['Y'], human['X'], human['T']], axis=1)  # Correct order: Y, X, T
            human_data = human_fixs if with_duration else human_fixs[:, :2]
            human_seq = scanmatch.fixationToSequence(human_data).astype(int)

            score, _, _ = scanmatch.match(human_seq, pred_seq)
            key = (task_name, img_name, condition, subject)
            results[key] = score
...
""" created: Dario Zanca, July 2017
    ...
    Implementation of the metric described in "Simulating Expert Saccadic 
...
def time_delay_embedding_distance(
    ...
    for s_k_vec in simulated_scanpath_vectors:
        # find expert k-vec of minimum distance
...
if __name__ == "__main__":
    ...
    sed = string_edit_distance(stimulus, human_fixations_list, simulated_fixations_list)
    stde = scaled_time_delay_embedding_similarity(
        human_fixations_list, simulated_fixations_list, stimulus
    )
